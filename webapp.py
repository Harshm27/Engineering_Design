...
from drawing2solid import builder, viewer, reader, verify as verifier
...
def esc0(x): return str(x).replace("&", "&amp;").replace("<", "&lt;")
...
def index():
  ...
  <form class="card" method="post" action="/read" enctype="multipart/form-data">
    ...
    <p>A dimensioned 2D drawing of a <b>turned part</b> (image or single-page PDF
    exported as an image). The AI reading step runs once, the builder checks the
    numbers close, and you get the dimensions, the spec and the 3D model back.
    Typically 30&ndash;60 seconds.</p>
    <label for="dwg">drawing image (png / jpg / webp)</label>
    <input type="file" id="dwg" name="dwg" accept="image/png,image/jpeg,image/webp" required>
    <div style="margin-top:14px"><button type="submit">Read the drawing</button></div>
    <div class="hint">Reading uses the Claude API and costs a few pence per drawing.</div>
  </form>

  <form class="card" method="post" action="/build" enctype="multipart/form-data">
    <h2>Or start from a spec</h2>
...
async def build(spec: str = Form(...), box: str = Form("280,140,900,440"),
                drawing: UploadFile = File(None), read_notes=None):
    ...
    if read_notes:
        out.append('<div class="card"><h2>Drawing read <span class="pill">'
                   + esc0(read_notes[0]) + '</span></h2>'
                   '<p>The spec below is what was read off your drawing. Check it against the '
                   'original; every number is editable on the front page.</p>'
                   f'<pre style="max-height:260px;overflow:auto">{esc0(json.dumps({k: s[k] for k in ("profile","features","checks") if k in s}, indent=1))}</pre></div>')
    rows = "".join(
...
async def read(dwg: UploadFile = File(...)):
    data = await dwg.read()
    if len(data) > 15_000_000:
        return HEAD + '<div class="card"><h2>Too large</h2><p>Keep the image under 15 MB.</p></div>' + FOOT
    mt = dwg.content_type if dwg.content_type in ("image/png","image/jpeg","image/webp") else "image/png"
    mock = None
    if os.environ.get("READER_MOCK"):                       # test path, no model call
        mock = json.loads(Path(os.environ["READER_MOCK"]).read_text())
    try:
        spec, attempts, notes = reader.read_drawing(data, mt, mock_spec=mock)
    except reader.ReadError as e:
        return HEAD + ('<div class="card"><h2>Could not read this drawing '
                       '<span class="pill warn">out of scope or unreadable</span></h2>'
                       f'<pre>{e}</pre><p class="note">Scope is turned parts: shafts, spools, '
                       'bushes, pins. Sheet-metal and milled parts need the manual route.</p>'
                       '<div class="dl"><a href="/">Back</a></div></div>') + FOOT
    except builder.ChainError as e:
        return HEAD + ('<div class="card"><h2>Read twice, still does not close '
                       '<span class="pill warn">refused, as designed</span></h2>'
                       '<p>The model read the drawing, the builder refused, the model re-read it, '
                       'and the numbers still contradict each other. Rather than hand you a wrong '
                       'part, here is the residual:</p>'
                       f'<pre>{e}</pre><p class="note">A sharper scan of the drawing usually fixes '
                       'this; so does reading it interactively in a Claude chat, where the views '
                       'can be examined one at a time.</p>'
                       '<div class="dl"><a href="/">Back</a></div></div>') + FOOT
    # a successful read: run the normal build path and show spec + dimensions
    return await build(spec=json.dumps(spec), box="", drawing=None, read_notes=notes)
